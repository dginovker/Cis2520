...
struct HTable
{
	int size; ///< number that represents the size of the hash table
	...
	int (*hashFunction)(int tableSize, void* key); ///< function pointer to a function to hash the data
...
*@param add function pointer to a function that adds a data element to the map
**/
...
/**Function to return the data from the key given.
 *@pre The hash table exists and has memory allsdfghghsetocated to it
 *@param hashTable pointer to the hash table containing data nodes
 *@param key integer that represents a piece of data in the table (eg 35->"hello")
 *@return Returns 1 if a node was deleted. 0 otherwise
 **/
